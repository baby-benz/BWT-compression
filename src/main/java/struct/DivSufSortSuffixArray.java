...
public final class DivSufSortSuffixArray {
    ...
    public DivSufSortSuffixArray() {
        this.bucketA = new int[Constants.ALPHABET_SIZE];
        this.bucketB = new int[Constants.ALPHABET_SIZE*Constants.ALPHABET_SIZE];
        this.sa = new int[0];
    ...
    private void constructSuffixArray(int[] bucketA, int[] bucketB, int n, int m) {
        ...
        int k = bucketA[c2];
        this.sa[k++] = (this.buffer[n - 2] < c2) ? -n : n - 1;

        for (int i = 0; i < n; i++) { // Сканируем суффиксный массив слева направо
            int s = this.sa[i];
    ...
    private int sortTypeBstar(int[] bucketA, int[] bucketB, final int n) {
        ...
        for (int i = 0, j = 0; c0 < Constants.ALPHABET_SIZE; c0++) {
            final int t = i + bucketA[c0];
            bucketA[c0] = i + j; // start point
            final int idx = c0 << 8;
            i = t + bucketB[idx + c0];

            for (int c1 = c0 + 1; c1 < Constants.ALPHABET_SIZE; c1++) {
                j += bucketB[idx + c1];
        ...
        if (m > 0) {
            ...
            for (int j = m; j > 0; c0--) {
                ...
                for (int c1 = 255; c1 > c0; c1--) {
                    ...
                    if (j > i + 1)
                        this.ssSort(pab, i, j, m, bufSize, n, arr[i] == m - 1);
            ...
            for (c0 = 254; c0 >= 0; c0--) {
                ...
                bucketB[idx + c0 + 1] = i - bucketB[idx + c0] + 1;
                bucketB[idx + c0] = i;
            }
        }
    ...
    private void ssSort(final int pa, int first, int last, int buf, int bufSize, int n, boolean lastSuffix) {
        if (lastSuffix) {
            first++;
        }
        ...
        for (a = first; middle - a > SS_BLOCKSIZE; a += SS_BLOCKSIZE, i++) {
            this.ssMultiKeyIntroSort(pa, a, a + SS_BLOCKSIZE);
            ...
            for (int j = i; (j & 1) != 0; j >>= 1) {
                this.ssSwapMerge(pa, b - k, b, b + k, curBuf, curBufSize);
                b -= k;
                k <<= 1;
            }
        }

        this.ssMultiKeyIntroSort(pa, a, middle);

        for (int k = SS_BLOCKSIZE; i != 0; k <<= 1, i >>= 1) {
            if ((i & 1) == 0)
                continue;

            this.ssSwapMerge(pa, a - k, a, middle, buf, bufSize);
            a -= k;
        }

        if (limit != 0) {
            this.ssMultiKeyIntroSort(pa, middle, last);
            this.ssInplaceMerge(pa, first, middle, last);
        }
        ...
        if (lastSuffix) {
            ...
            for (a = first; (a < last) && ((this.sa[a] < 0) || (this.ssCompare(p1, p11, pa + this.sa[a], 2) > 0)); a++)
    ...
    private void ssInplaceMerge(int pa, int first, int middle, int last) {
        final int[] arr = this.sa;
        ...
        while (true) {
            ...
            for (int len = middle - first, half = (len >> 1); len > 0; len = half, half >>= 1) {
                final int b = a + half;
                final int q = ssCompare(pa + ((arr[b] >= 0) ? arr[b] : ~arr[b]), p, 2);
        ...
        while (true) {
            if (last - middle <= bufSize) {
                if ((first < middle) && (middle < last))
                    this.ssMergeBackward(pa, first, middle, last, buf);
                ...
                if (((check & 4) != 0)
                        && ((this.ssCompare(pa + getIndex(arr[last - 1]), pa + arr[last], 2) == 0))) {
                    arr[last] = ~arr[last];
                }
            ...
            if (middle - first <= bufSize) {
                if (first < middle)
                    this.ssMergeForward(pa, first, middle, last, buf);
                ...
                if (((check & 4) != 0)
                        && ((ssCompare(pa + getIndex(arr[last - 1]), pa + arr[last], 2) == 0))) {
                    arr[last] = ~arr[last];
                }
            ...
            for (int half = len >> 1; len > 0; len = half, half >>= 1) {
                if (ssCompare(pa + getIndex(arr[middle + m + half]), pa + getIndex(arr[middle - m - half - 1]), 2) < 0) {
                    m += (half + 1);
                    half -= ((len & 1) ^ 1);
                }
            }
            ...
            if (m > 0) {
                ...
                if (this.ssCompare(pa + getIndex(arr[middle - 1]), pa + arr[middle], 2) == 0) {
    ...
    private void ssMergeForward(int pa, int first, int middle, int last, int buf) {
        ...
        while (true) {
            final int r = ssCompare(pa + arr[b], pa + arr[c], 2);
    ...
    private void ssMergeBackward(int pa, int first, int middle, int last, int buf) {
        ...
        while (true) {
            final int r = this.ssCompare(p1, p2, 2);
    ...
    private void ssMultiKeyIntroSort(final int pa, int first, int last) {
        ...
        while (true) {
            ...
            if (a <= d) {
                c = b - 1;
                int s = Math.min(a - first, b - a);

                for (int e = first, f = b - s; s > 0; s--, e++, f++) {
                    this.swapInSA(e, f);
                }

                s = Math.min(d - c, last - d - 1);
    ...
    // Tandem Repeat Sort
    private void trSort(int n) {
        final int[] arr = this.sa;
        TRBudget budget = new TRBudget(trIlg(n) * 2 / 3, n);

        for (int isad = n + 1; arr[0] > -n; isad += (isad - n)) {
            int first = 0;
            int skip = 0;
            int unsorted = 0;
